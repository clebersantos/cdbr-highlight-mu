...
class WidgetHighlightsMU extends WP_Widget
{
	...
	function widget( $args, $instance )
	{
		...
		if( $highlights ) {
			...
			print '<div id="highlights" class="two column doubling ui grid">';
				...
				// print $title;
			 	...
			 	if( is_array( $highlights ) ) {
					...
					foreach( $highlights as $highlight ) { 
						...
						// adaptação para os destaques utilizando semantic ui no Cultura Digital.br - em 25/09/2014
						...
						<?php if( function_exists( 'switch_to_blog' ) ) switch_to_blog( $highlight[ 'blog_id' ] ); ?>
								...
								<?php if( $cycle->have_posts() ) : $cycle->the_post(); ?>
									...
									<div id="post-<?php print get_the_ID(); ?>" class="<?php print strlen( $highlight[ 'highlight_excerpt' ]) <= 1  ? 'not-excerpt' : 'true-excerpt';  ?>">
										<div class="pic">
											<a href="<?php print get_permalink(); ?>" title="<?php print $highlight[ 'highlight_title' ]; ?>"><?php the_post_thumbnail( "headline" ); ?></a>
										</div>
										...
										<div class="headline">
											...
											<div class="post-title"><a href="<?php print get_permalink(); ?>" title="<?php print get_the_title(); ?>"><?php print limit_chars( $highlight[ 'highlight_title' ], $limit_title ); ?></a></div>
											<div class="post-excerpt"><?php print limit_chars( $highlight[ 'highlight_excerpt' ], $limit_excerpt ); ?></div>
										</div>
									</div>
